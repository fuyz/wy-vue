...
<template>
    ...
    <mt-navbar id="navBar" v-model="navSelected" fixed>
        <mt-tab-item :id="item" v-for="(item, index) in navBarList" :key="index" :href="'#'+ item.link">
          {{ item.title }}
        </mt-tab-item>
    </mt-navbar>
</template>
...
//          {link: '/video/videoList/二次元', title: '二次元' },
    ...
    created: function () {
      ...
      this.navSelected = this.$route.params.type ? this.$route.params.type : this.navSelected;

      this.$http.jsonp(this.host_port + '?key=wy&url=' + this.videoNav_url ).then((res) => {
        try{
          ...
          res.forEach(function (e, i) {
            ...
            URL_PARAMS.videoUrl[e.cname] = 'http://c.m.163.com/recommend/getChanListNews?channel=T1457068979049&subtab='+  e.ename +'&size=10&offset=0&fn=1'
          }.bind(this));
